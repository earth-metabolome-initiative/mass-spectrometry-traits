...
fn count_edges() {
    ...
    let epi: GenericSpectrum =
        GenericSpectrum::epimeloscine().expect("reference spectrum should build");
    let tol = 0.1f64;

    type SpectrumPair<'a> = (&'a str, &'a GenericSpectrum, &'a str, &'a GenericSpectrum);
