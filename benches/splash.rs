...
fn bench_splash(c: &mut Criterion) {
    ...
    for row in &rows {
        assert_eq!(
            splash_from_peaks(&row.peaks).expect("reference row should produce SPLASH"),
            row.expected
        );
    }

    let peak_count = rows.iter().map(|row| row.peaks.len()).sum::<usize>();
    ...
    group.bench_function(
        format!(
            "splash_from_peaks_{}_spectra_{}_peaks",
            rows.len(),
            peak_count
        ),
        |b| {
            b.iter(|| {
                ...
                for row in &rows {
                    let splash = splash_from_peaks(black_box(&row.peaks))
    ...
    group.bench_function(
        ...
        |b| {
            b.iter(|| {
                let mut bytes = 0usize;
                for row in &rows {
                    let peaks = parse_spectrum(black_box(row.spectrum.as_str()));
                    let splash =
                        splash_from_peaks(&peaks).expect("reference row should produce SPLASH");
                    bytes += black_box(splash).len();
                }
                black_box(bytes);
            });
